...
///
class AliFemtoTrackMassCutAK : public AliFemtoTrackCutPionPionAK {
public:

  double target_mass = 0.139;

  AliFemtoTrackMassCutAK()
    : AliFemtoTrackCutPionPionAK()
    {}

  AliFemtoTrackMassCutAK(double mass)
    : AliFemtoTrackCutPionPionAK()
    , target_mass(mass)
    {}

  AliFemtoTrackMassCutAK(AliFemtoConfigObject &cfg)
    : AliFemtoTrackCutPionPionAK(cfg)
    {}

  AliFemtoTrackMassCutAK(double mass, AliFemtoConfigObject &cfg)
    : AliFemtoTrackCutPionPionAK(cfg)
    , target_mass(mass)
    {}

  virtual ~AliFemtoTrackMassCutAK() {}

  virtual bool Pass(const AliFemtoTrack *track)
    {
      const auto *info = static_cast<AliFemtoModelHiddenInfo*>(track->GetHiddenInfo());

      if (0.0 == info->GetTrueMomentum()->Mag2()) {
        return false;
      }

      if (true) {
        const double mass = info->GetMass();
        if (std::abs(mass - target_mass) > .01) {
          // std::cout << "MASS OUTSIDE RANGE: " << mass << "\n";
          return false;
        }
      } else {
        // const double mass = info->GetTrueMomentum();
        // if (std::abs(mass - target_mass) > .01) {
          // std::cout << "MASS OUTSIDE RANGE: " << mass << "\n";
          // return false;
        // }
  ...
  virtual const char* ClassName() const
    { return "AliFemtoTrackMassCutAK"; }
};


class MyTrackCut : public AliFemtoESDTrackCut {
public:
  ...
  MyTrackCut()
    : AliFemtoESDTrackCut()
    {
      ...
      SetMostProbable(2);
      SetPIDMethod(AliFemtoESDTrackCut::knSigma);
      SetStatus(16);
...
/// Cut on MonteCarlo PDG code to select *only* pions
///
class AliFemtoSpecialPairCutAK : public AliFemtoPairCutPionPionAKDetaDphi {
public:

  AliFemtoSpecialPairCutAK()
    : AliFemtoPairCutPionPionAKDetaDphi()
    {}

  AliFemtoSpecialPairCutAK(AliFemtoConfigObject &cfg)
    : AliFemtoPairCutPionPionAKDetaDphi(cfg)
    {}

  AliFemtoSpecialPairCutAK(double mass, AliFemtoConfigObject &cfg)
    : AliFemtoPairCutPionPionAKDetaDphi(cfg)
    {}

  virtual ~AliFemtoSpecialPairCutAK() {}
  ...
  virtual bool Pass(const AliFemtoPair *pair)
    {
      ...
      if (info1.GetMass() == 0.0 || info2.GetMass() == 0.0) {
        return false;
      }

      if (!(abs(info1.GetPDGPid()) == 211 || abs(info2.GetPDGPid()) == 211)) {
        return false;
      }

      // muon
      // if (!(abs(info1.GetPDGPid()) == 211 && abs(info2.GetPDGPid()) == 13) ||
      //      (abs(info1.GetPDGPid()) == 13 && abs(info2.GetPDGPid()) == 211))
      //   {
      //     return false;
      //   }

      // kaon
      // if (!(abs(info1.GetPDGPid()) == 211 && abs(info2.GetPDGPid()) == 321) ||
      //     (abs(info1.GetPDGPid()) == 321 && abs(info2.GetPDGPid()) == 211))
      //   {
      //     return false;
      //   }

      // if (!(abs(info1.GetPDGPid()) == 211 && abs(info2.GetPDGPid()) == 2212) ||
      //     (abs(info1.GetPDGPid()) == 2212 && abs(info2.GetPDGPid()) == 211))
      //   {
      //     return false;
      //   }

      const AliFemtoThreeVector
        &p1_ideal = *info1.GetTrueMomentum(),
        &p2_ideal = *info2.GetTrueMomentum();
      ...
      AliFemtoLorentzVector
        p1(p1_ideal, p1_ideal.MassHypothesis(mass1)),
        p2(p2_ideal, p2_ideal.MassHypothesis(mass2));

      const Double_t
        qgen = std::fabs((p1 - p2).m()),
        qrec = pair->QInv();
  ...
  virtual const char* ClassName() const
    { return "AliFemtoSpecialPairCutAK"; }
};
...
struct MacroParams : public TNamed {
  ...
  bool do_trueqinv_cf { true };
  bool do_kt_trueqinv_cf { true };
...
ConfigFemtoAnalysis(const TString& param_str="")
{
  ...
  analysis_config.is_mc_analysis = true;
  ...
  if (macro_config.centrality_ranges.empty()) {
    ...
    macro_config.centrality_ranges.push_back(1.0);
    macro_config.centrality_ranges.push_back(12.0);
  }
  ...
  macro_config.kt_ranges = {0.2, 0.3, 0.4, 0.7, 0.8, 1.2};
  ...
  for (int cent_it = 0; cent_it + 1 < macro_config.centrality_ranges.size(); cent_it += 2) {
    ...
    for (int pair_it = 0; pair_it < macro_config.pair_codes.size(); ++pair_it) {
      ...
      auto trackcfg = AliFemtoConfigObject::Parse(R"( { charge: 1, status: 16, pt_range: 0.2:2.0, nsigma_pion: 3.0, max_z: 0.1, max_xy: 0.1, remove_neg_label: true } )");
      // auto *trackcut = new AliFemtoTrackCutPionPionAK(trackcfg);
      auto *trackcut = new AliFemtoTrackCutPionPionIdealAK(trackcfg);
      // // auto *trackcut = new MyTrackCut();
      analysis->SetFirstParticleCut(trackcut);
      analysis->SetSecondParticleCut(trackcut);

      auto paircfg = AliFemtoConfigObject::Parse(R"( { share_fraction_max: 0.05, delta_eta_min: 0.01, delta_phistar_min: 0.01 } )");
      auto *paircut = new AliFemtoSpecialPairCutAK(paircfg);
      analysis->SetPairCut(paircut);

      std::cout << " > " << analysis->GetConfiguration().Stringify() << "\n";
